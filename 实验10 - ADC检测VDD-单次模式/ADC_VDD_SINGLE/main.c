...
** Last Version:		   1.2
** Descriptions: 每隔500ms，nRF24LE1对VDD进行一次采样，并将采样值通过串口输出	
** ADC配置：
**   基准电压：内部1.2V
**   通道：检测1/3 VDD电压
**   分辨率：12位
...
void AdcInit(void)
{
  ...
  hal_adc_set_reference(HAL_ADC_REF_INT);             // 设置参考电压 内部1.2V                  
...
void main()
{
  ...
  while(1)
  {
	  D1 = ~D1;       //D4闪烁指示ADC转换
		...
		hal_adc_start();         // 启动ADC
		while( hal_adc_busy())   // 等待转换结束
		  ;
		PutString("ADCC Samping Value(HEX):");		  
		PutHexString(hal_adc_read_MSB());      //串口输出ADC转换结果
		PutHexString(hal_adc_read_LSB());
		PutString("\n");
  }
} 
/*****************************************End File***********************************************************/
